...
namespace Alunos.Controller
{
    ...
    public class AlunoController : ControllerBase
    {
        ...
        [Route("api/[controller]/AtualizarPagamento")]
        [HttpPost]
        public IActionResult EstadoPagamento([FromBody] AlunoPagamentoUpdateRequest request)
        {
            try
            {
                var aluno = _alunoService.BuscarPorId(request.Id);

                if (Enum.IsDefined(typeof(EnumStatusMensalidade), request.PagamentoStatus))
                {
                    aluno.StatusMensalidade = (EnumStatusMensalidade)request.PagamentoStatus;
                }
                else
                {
                    return BadRequest("Status inválido.");
